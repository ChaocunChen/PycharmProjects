...
from skimage.feature import hog,hessian_matrix_eigvals
import numpy as np
import os
import joblib
...
# 训练集图片的位置
train_image_path = 'resize_img'
# 测试集图片的位置
test_image_path = 'resize_img'

# 训练集标签的位置
train_label_path = os.path.join('./','train.txt')
# 测试集标签的位置
test_label_path = os.path.join('./','train.txt')
LinearSVC()
image_height = 112
image_width = 112
test_image_path = 'resize_img'
...
def rgb2gray(im):
    ...
    return gray
img_list = []
nameList= os.listdir(test_image_path)
for name in nameList:
    temp = Image.open(os.path.join(test_image_path,name))
    img_list.append(temp.copy())
    for image_name in img_list:
        image = np.reshape(image_name, (image_height, image_width, 3))
...
def train_and_test():
    ...
    for feat_path in glob.glob(os.path.join(test_feat_path, '*.feat')):
        ...
        if platform.system() == 'Windows':
            ...
            symbol = '/'
        image_name = feat_path.split(symbol)[5].split('.feat')[0]
