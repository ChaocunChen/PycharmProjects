import sys,argparse
sys.path.insert(0, "/home/chenchaocun/caffe_ssd_CenterLoss/python")
import caffe
caffe.set_mode_cpu()
import numpy as np
...
def print_net_parameters_flops (arg):
    print ("Net: " + arg.deploy)
    net = caffe.Net(arg.deploy, caffe.TEST)
    flops = 0
    typenames = ['Convolution', 'DepthwiseConvolution', 'InnerProduct']

    print ("Layer-wise parameters: ")
    print ('layer name'.ljust(20), 'Filter Shape'.ljust(20),'Output Size'.ljust(20), 'Layer Type'.ljust(20), 'Flops'.ljust(20))
    ...
    for layer_name, blob in net.blobs.items():
        ...
        if net.layer_dict[layer_name].type in typenames:
            ...
            if net.layer_dict[layer_name].type in typenames[:2]:
                cur_flops = (np.product(net.params[layer_name][0].data.shape) * blob.data.shape[-1] * blob.data.shape[-2])
            else:
...
if __name__ == '__main__':
    print_net_parameters_flops(parse_args())